...
async function fetchShaderCode(matid, backend, language, index) {
    ...
    return _fetchText(`api/shader?matid=${matid}&${query}`);
}

async function fetchMaterials() {
    return _fetchJson("api/materials").then((matJson) => {
        let ret = {};
...
async function fetchMaterial(matId) {
    return _fetchJson(`api/material?matid=${matid}`).then((matInfo) => {
        matInfo.matid = matid;
        return matInfo;
    });
}
...
async function fetchMatIds() {
    return _fetchJson("api/matids").then(function(matInfo) {
        let ret = [];
...
function queryActiveShaders() {
    return _fetchJson("api/active").then(function(activeMaterials) {
        let actives = {};
...
async function rebuildMaterial(materialId, backend, shaderIndex, editedText) {
    let api = 0;
    switch (backend) {
...
function statusLoop(isConnected, onStatus) {
    // This is a hanging get except for when transition from disconnected to connected, which
    // should return immediately.
    _fetchText("api/status" + (isConnected() ? '' : '?firstTime'))
        .then((response) => {
            const matid = response;
